...
- **Cross-Plugin Health Snapshot**: Automatically exports an atomic `McpStatusSnapshot` via `PluginStorage` (`~/.paseo/mcp-tools/status.json`) on every health poll, enabling other plugins (such as `paseo-top`) to display MCP server health with zero duplicated probes.
